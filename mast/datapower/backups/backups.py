...
# set-checkpoint - Sets a checkpoint in the given domain on the given appliances


@logged("mast.datapower.backups")
@cli.command('restore-normal-backup', category='backups')
def restore_normal_backup(appliances=[], credentials=[], timeout=120,
    file_in=None, Domain="", source_type="ZIP", overwrite_files=True,
    overwrite_objects=True, rewrite_local_ip=True, deployment_policy=None,
    import_domain=True, reset_domain=True, dry_run=False, out_dir="tmp",
...
def get_normal_backup(appliances=[], credentials=[],
                  timeout=120, Domain=[],
                  comment="", out_dir='tmp', no_check_hostname=False, web=False):
...
"""
    ...
    for domain in Domain:
        logger.info("Attempting to retrieve normal backup from {} in {} domain".format(str(env.appliances), domain))
...
the DataPower
    ...
    if web:
        output += util.render_boolean_results_table(resp, suffix="SecureBackup")
    ...
    for appliance in env.appliances:
        ...
        start = time()
        while not appliance.file_exists('{}/backupmanifest.xml'.format(
            destination), 'default'):
                sleep(5)
        ...
        try:
            ...
            if appliance.verify_local_backup(_directory):
                ...
                if remove:
                    logger.info(
                        "Attempting to remove Secure Backup from appliance {}".format(
                            appliance.hostname))
...
def restore_secure_backup(appliances=[], credentials=[], timeout=1200,
    CryptoCertificate="", location="", validate=True, no_check_hostname=False, web=False):
...
def list_checkpoints(appliances=[], credentials=[], timeout=120,
    ...
    """Lists the checkpoints which are currently in the specified domain"""
    logger = make_logger("mast.backups")
    check_hostname = not no_check_hostname
    env = datapower.Environment(appliances, credentials, timeout, check_hostname=check_hostname)
    logger.info(
    ...
    logger.debug("Responses received: {}".format(str(resp)))
    if web:
        return util.web_list_checkpoints(resp, Domain), util.render_history(env)
...
def remove_checkpoint(appliances=[], credentials=[], timeout=120,
    Domain="", checkpoint_name="", no_check_hostname=False, web=False):
...
* checkpoint_name - The name of the checkpoint to remove"""
    ...
    check_hostname = not no_check_hostname
    env = datapower.Environment(appliances, credentials, timeout, check_hostname=check_hostname)
    logger.info("Attempting to remove checkpoint {} from {} in {} domain".format(
        checkpoint_name,
        str(env.appliances),
        Domain))
...
def set_checkpoint(appliances=[], credentials=[],
                   ...
                   comment='', remove_oldest=True, no_check_hostname=False, web=False):
        ...
        for domain in Domain:
            ...
            if remove_oldest:
                ...
                if len(appliance.get_existing_checkpoints(domain)) >= _max:
                    logger.info("Maximum number of checkpoints for domain {} on {} reached. Removing oldest checkpoint.".format(domain, appliance.hostname))
